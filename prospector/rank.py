...
seconds_per_month = (
    31557600 / 12
)  # dividing the vulnerability published timestamp by this number to "normalize" it a bit
...
reference_stopwords = [
    ...
    "version",
    "release",
] + fix_indicating_words  # words to avoid sampling
...
def filter_text(
    ...
    for chunk in text_into_chunks(text, chunk_size=10000):
        for token in nlp(chunk):
            if (
                ...
                and token.pos_ in ["VERB", "NOUN", "PROPN", "ADJ"]
                and any(char for char in token.text if char.isalpha())
                and len(token) > 1
            ):
    ...
    for token in tokens:
        if camel_case_split(token):
            if lemmatize:
                result += [
                    camel_case_token.lemma_
                    for camel_case_token in nlp(" ".join(camel_case_split(token)))
                ]
            else:
            ...
            if lemmatize:
                result += [
                    snake_case_token.lemma_
                    for snake_case_token in nlp(" ".join(snake_case_split(token)))
                ]
            else:
            ...
            if lemmatize:
                result += [
                    dot_case_token.lemma_
                    for dot_case_token in nlp(" ".join(dot_case_split(token)))
                ]
            else:
...
def filter_doc(doc):
    ...
    if type(doc) != spacy.tokens.doc.Doc:
        raise TypeError(
            "The document should be a spacy.tokens.doc.Doc, which is created by means of nlp("
        )
    ...
    tokens = [
        ...
        and any(char for char in token.text if char.isalpha())
        and len(token) > 1
    ]  # token.pos_ in ['VERB', 'NOUN', 'PROPN', 'ADJ'] and
    result = list()
    for token in tokens:
        if camel_case_split(token.text):
            result += [
                camel_case_token.lemma_
                for camel_case_token in nlp(" ".join(camel_case_split(token.text)))
            ]
        elif snake_case_split(token.text):
            result += [
                snake_case_token.lemma_
                for snake_case_token in nlp(" ".join(snake_case_split(token.text)))
            ]
        elif dot_case_split(token.text):
            result += [
                dot_case_token.lemma_
                for dot_case_token in nlp(" ".join(dot_case_split(token.text)))
            ]
...
def simpler_filter_text(text):
    ...
    # filter text, needs to be in chunks due to spacy maximum of 1000000 characters
    return " ".join(
        [filter_doc(nlp(chunk)) for chunk in text_into_chunks(text, chunk_size=10000)]
    ).lower()
...
def extract_relevant_lines_from_commit_diff(git_diff, max_lines=10000):
    ...
    return [
        ...
        if line.startswith(("diff --git", "index ", "+++ ", "--- ", "@@ ")) == False
    ][:max_lines]
...
def extract_n_most_occurring_words_from_references(
    references, repo_url=None, n=20, return_urls=False, driver=None
):
    """
                    ...
                    bow += filter_text(
                        " ".join(
                            [
                                ...
                                if string not in strings_on_every_page
                            ]
                        ),
                        as_list=True,
                    )
...
def extract_project_name_from_repository_url(repo_url):
    ...
    return " ".join(
        [
            token
            for token in project_name.split(" ")
            if token not in ["github", "com", "git", "org"]
        ]
    )


def map_description_to_repository_url(
    ...
    vulnerabilities_df = pd.read_sql(
        "SELECT vulnerability_id, repo_url FROM vulnerabilities",
        vulnerabilities_connection,
    ).set_index("vulnerability_id")

    if vulnerability_id in tuple(vulnerabilities_df.index):
        return vulnerabilities_df.at[vulnerability_id, "repo_url"]

    vulnerabilities_df["project_name"] = vulnerabilities_df["repo_url"].apply(
        extract_project_name_from_repository_url
    )
    vulnerabilities_df["project_name"] = vulnerabilities_df["project_name"].apply(
        simpler_filter_text
    )
    ...
    preprocessed_description = simpler_filter_text(
    ...
    scores = {
        repo_url: cosine_similarity(
            tfidf_vectorized_strings[0], tfidf_vectorized_strings[i + 1]
        )[0][0]
...
class Advisory_record:
    ...
    def analyse_references(self):
        ...
        for reference in [
            self.Reference(reference, self.repo_url)
            for reference in self.nvd_references
        ]:
            ...
            if reference.is_commit_page():
                commit_id = reference.url.split("/commit/")[1][
                    :8
        ...
        return pd.DataFrame(
            {
                ...
                "total_n_commits_found": [
                    len(
                        list(
                            dict.fromkeys(
                                self.commits_found_nvd + self.commits_found_adv
                            )
                        )
                    )
                ],
    ...
    def validate_database_coverage(self):
        ...
        database.add_repository_to_database(
            self.connection, self.repo_url, self.project_name, verbose=self.verbose
        )
        database.add_tags_to_database(
            self.connection, tags=None, git_repo=self.git_repo, verbose=self.verbose
        )

        # remove candidates that are not in the databases
        cursor = self.connection.cursor()
        cursor.execute(
            "SELECT id FROM commits WHERE id IN {} and repository_url = :repo_url".format(
                tuple(self.candidate_commits + [self.candidate_commits[0]])
            ),
            {"repo_url": self.repo_url},
        )  # +[self.candidate_commits[0]] as SQL requires at least 2 items
        commits_in_the_db = [row["id"] for row in cursor]

        if self.verbose:
            print(
                "    Had to discard {} candidates as they were not in the DB.".format(
                    len(
                        [
                            commit_id
                            for commit_id in self.candidate_commits
                            if commit_id not in commits_in_the_db
                        ]
                    )
                )
            )
    ...
    def gather_candidate_commits(self):
        ...
        if self.since == None and self.until == None:
            self.since, self.until = database.timestamp_to_timestamp_interval(
                self.published_timestamp, days_before=days_before, days_after=days_after
            )
        ...
        if len(self.candidate_commits) > 0:
            ...
            self.candidate_commits = filter.filter_commits_on_files_changed_extensions(
                self.candidate_commits, self.connection, verbose=self.verbose
            )
        else:
            print("No candidates found.")

    def compute_ranking_vectors(self, vulnerability_specific_scaling=False):
        self.ranking_vectors = compute_ranking_vectors_for_advisory_records_with_db(
            self, vulnerability_specific_scaling
        )
    ...
    try:
        return " ".join(
            [
                ...
                if token not in project_name.split(" ")
            ]
        )
    except:
        return string
...
def check_preprocessing(advisory_record, candidate_commit_df):
    ...
    for index in candidate_commit_df[
        (candidate_commit_df.preprocessed_message == None)
        | (candidate_commit_df.preprocessed_message == "")
    ].index:
        preprocessed_message = simpler_filter_text(
            ast.literal_eval(candidate_commit_df.at[index, "message"])
        )
        candidate_commit_df.at[index, "preprocessed_message"] = preprocessed_message

    for index in candidate_commit_df[
        (candidate_commit_df.preprocessed_changed_files == None)
        | (candidate_commit_df.preprocessed_changed_files == "")
    ].index:
        preprocessed_message = simpler_filter_text(
            ast.literal_eval(candidate_commit_df.at[index, "changed_files"])
        )
        candidate_commit_df.at[
            index, "preprocessed_changed_files"
        ] = preprocessed_message

    for index in candidate_commit_df[
        (candidate_commit_df.preprocessed_diff == None)
        | (candidate_commit_df.preprocessed_diff == "")
    ].index:
        preprocessed_message = simpler_filter_text(
            ast.literal_eval(candidate_commit_df.at[index, "diff"])
        )
        candidate_commit_df.at[index, "preprocessed_diff"] = preprocessed_message

    candidate_commit_df["preprocessed_message"] = candidate_commit_df[
        "preprocessed_message"
    ].apply(remove_project_name_from_string, project_name=project_name)
    candidate_commit_df["preprocessed_changed_files"] = candidate_commit_df[
        "preprocessed_changed_files"
    ].apply(remove_project_name_from_string, project_name=project_name)
    candidate_commit_df["preprocessed_diff"] = candidate_commit_df[
        "preprocessed_diff"
    ].apply(remove_project_name_from_string, project_name=project_name)
...
def remove_forbidden_words_from_string(string, forbidden_words):
    ...
    try:
        return " ".join(
            [token for token in string.split(" ") if token not in forbidden_words]
        )
    except:
        return string
...
def compute_lexical_similarity_components(advisory_record, candidate_commit_df):
    ...
    description = remove_project_name_from_string(
        string=description, project_name=project_name
    )
    description_with_fix_indicating_words = description + " ".join(
        fix_indicating_words
    )  # for the commit message

    # CODE TOKENS
    code_tokens = simpler_filter_text(extract_code_tokens(advisory_record.description))
    code_tokens = remove_project_name_from_string(
        string=code_tokens, project_name=project_name
    )

    # define the different aspects and compute vector components
    tfidf_vectorizer_messages, tfidf_vectorizer_files, tfidf_vectorizer_diff = (
        TfidfVectorizer(),
        TfidfVectorizer(),
        TfidfVectorizer(),
    )

    tfidf_messages = tfidf_vectorizer_messages.fit_transform(
        [
            description_with_fix_indicating_words,
            advisory_record.references_content,
            code_tokens,
        ]
        + list(candidate_commit_df["preprocessed_message"])
    )
    tfidf_files = tfidf_vectorizer_files.fit_transform(
        [description, advisory_record.references_content, code_tokens]
        + list(candidate_commit_df["preprocessed_changed_files"])
    )
    tfidf_diffs = tfidf_vectorizer_diff.fit_transform(
        [description, advisory_record.references_content, code_tokens]
        + list(candidate_commit_df["preprocessed_diff"])
    )
    ...
    if with_multiprocessing:
        with Pool(number_of_cpus) as p:
            ...
            candidate_commit_df["message_score"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_messages[0], tfidf_messages[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )
            candidate_commit_df["changed_files_score"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_files[0], tfidf_files[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )
            candidate_commit_df["git_diff_score"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_diffs[0], tfidf_diffs[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )

            # with nvd reference content
            candidate_commit_df["message_score_reference_content"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_messages[1], tfidf_messages[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )
            candidate_commit_df["changed_files_score_reference_content"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_files[1], tfidf_files[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )
            candidate_commit_df["git_diff_score_reference_content"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_diffs[1], tfidf_diffs[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )

            # with code tokens
            candidate_commit_df["message_score_code_tokens"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_messages[2], tfidf_messages[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )
            candidate_commit_df["changed_files_score_code_tokens"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_files[2], tfidf_files[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )
            candidate_commit_df["git_diff_score_code_tokens"] = p.starmap(
                get_cosine_similarity,
                [
                    (tfidf_diffs[2], tfidf_diffs[index + 3])
                    for index in range(len(candidate_commit_df))
                ],
            )
    else:
        # vulnerability description
        candidate_commit_df["message_score"] = candidate_commit_df.apply(
            lambda x: cosine_similarity(tfidf_messages[0], tfidf_messages[x.name + 3])[
                0
            ][0],
            axis=1,
        )  # x.name is index
        candidate_commit_df["changed_files_score"] = candidate_commit_df.apply(
            lambda x: cosine_similarity(tfidf_files[0], tfidf_files[x.name + 3])[0][0],
            axis=1,
        )
        candidate_commit_df["git_diff_score"] = candidate_commit_df.apply(
            lambda x: cosine_similarity(tfidf_diffs[0], tfidf_diffs[x.name + 3])[0][0],
            axis=1,
        )
        ...
        ] = candidate_commit_df.apply(
            lambda x: cosine_similarity(tfidf_messages[1], tfidf_messages[x.name + 3])[
                0
            ][0],
            axis=1,
        )  # x.name is index
        candidate_commit_df[
            ...
            lambda x: cosine_similarity(tfidf_files[1], tfidf_files[x.name + 3])[0][0],
            axis=1,
        )
        candidate_commit_df[
            "git_diff_score_reference_content"
        ] = candidate_commit_df.apply(
            lambda x: cosine_similarity(tfidf_diffs[1], tfidf_diffs[x.name + 3])[0][0],
            axis=1,
        )

        # with code tokens
        candidate_commit_df["message_score_code_tokens"] = candidate_commit_df.apply(
            lambda x: cosine_similarity(tfidf_messages[2], tfidf_messages[x.name + 3])[
                0
            ][0],
            axis=1,
        )  # x.name is index
        candidate_commit_df[
            ...
            lambda x: cosine_similarity(tfidf_files[2], tfidf_files[x.name + 3])[0][0],
            axis=1,
        )
        candidate_commit_df["git_diff_score_code_tokens"] = candidate_commit_df.apply(
            lambda x: cosine_similarity(tfidf_diffs[2], tfidf_diffs[x.name + 3])[0][0],
            axis=1,
        )
    return candidate_commit_df
...
def extract_path_tokens_from_text(text):
    ...
    """
    return [
        re.split("\.|,|/", token.rstrip(".,;:?!\"'"))
        for token in text.split(" ")
        if ("/" in token.rstrip(".,;:?!\"'") and not token.startswith("</"))
        or (
            "." in token.rstrip(".,;:?!\"'")
            and token.rstrip(".,;:?!\"'").split(".")[-1] in relevant_extensions
        )
...
def compute_path_similarity_score(changed_files, path_tokens_list):
    ...
    for path_tokens in path_tokens_list:
        ...
        with_extension = (
            True if reversed_path_tokens[0] in relevant_extensions else False
        )

        for changed_file in changed_files:

            changed_file_tokens = list(
                reversed([token.lower() for token in re.split("\.|,|/", changed_file)])
            )
            ...
            while (
                same_tokens == True
                and same_token_count < len(reversed_path_tokens)
                and same_token_count < len(changed_file_tokens)
            ):
                if (
                    reversed_path_tokens[same_token_count]
                    == changed_file_tokens[same_token_count]
                ):
                    same_token_count += 1
                else:
                    same_tokens = False
...
def calculate_time_distance_before(
    commit_timestamp, published_timestamp, first_candidate_timestamp
):
    """
    ...
    return (
        0.5
        + (
            (commit_timestamp - first_candidate_timestamp)
            / (published_timestamp - first_candidate_timestamp)
        )
        / 2
        if commit_timestamp < published_timestamp
        else 0.0
    )


def calculate_time_distance_after(
    commit_timestamp, published_timestamp, last_candidate_timestamp
):
    """
    Used to compute a feature reflecting the distance from a commit and the vulnerability publication data,
    ...
    return (
        1
        - (
            (commit_timestamp - published_timestamp)
            / (last_candidate_timestamp - published_timestamp)
        )
        / 2
        if commit_timestamp > published_timestamp
        else 0.0
...
def find_next_tag_in_tags_tree(
    tag, sorted_tags_tree, tag_timestamp_dict, tag_timestamp=None, first_call=True
):
    """
    ...
    if (
        sorted_tags_tree[major][minor].index(tag)
        < len(sorted_tags_tree[major][minor]) - 1
    ):
        return sorted_tags_tree[major][minor][
            sorted_tags_tree[major][minor].index(tag) + 1
        ]
    ...
    if tuple(sorted_tags_tree[major]).index(minor) < len(sorted_tags_tree[major]) - 1:
        next_minor = tuple(sorted_tags_tree[major])[
            tuple(sorted_tags_tree[major]).index(minor) + 1
        ]
        ...
        # try again for the possible next tag
            ...
            return find_next_tag_in_tags_tree(
                possible_next_tag,
                sorted_tags_tree,
                tag_timestamp_dict,
                tag_timestamp,
...
def calculate_reachability_score(
    row, tag_timestamp_dict, reachable_commits, relevant_tags, days_before
):
    """
    ...
    return (
        ...
        - (
            min(
                [
                    int((int(tag_timestamp_dict[tag]) - int(row["timestamp"])) / 86400)
                    if row["id"] in reachable_commits[tag]
                    else days_before
...
def compute_commit_distance_components(
    advisory_record, candidate_commit_df, days_before=100
):
    """
    ...
    if advisory_record.relevant_tags == None:
        cursor.execute(
            "SELECT tag FROM tags WHERE repo_url = :repo_url",
        ...
        if len(tags) == 0:
            ...
            tags_in_description = [
                ...
                for tag in filter.get_tag_for_version(tags, version)
            ]
    ...
    candidate_commit_df["time_distance_before"] = candidate_commit_df[
        "timestamp"
    ].apply(
        calculate_time_distance_before,
        published_timestamp=int(advisory_record.published_timestamp),
        first_candidate_timestamp=first_candidate_timestamp,
    )

    last_candidate_timestamp = max(candidate_commit_df.timestamp)
    candidate_commit_df["time_distance_after"] = candidate_commit_df["timestamp"].apply(
        calculate_time_distance_after,
        published_timestamp=int(advisory_record.published_timestamp),
    ...
    if len(advisory_record.relevant_tags) == 0:
        ...
        candidate_commit_df["reachability_score"] = [
            0.0 for i in range(len(candidate_commit_df))
        ]
        return candidate_commit_df
    ...
    cursor.execute(
        "SELECT tag, tag_timestamp FROM tags WHERE repo_url = :repo_url",
        {"repo_url": advisory_record.repo_url},
    )
    tag_timestamp_dict = {row["tag"]: row["tag_timestamp"] for row in cursor}

    tags_to_calculate_reachability_from = tuple(
        [
            map_tag_onto_next_tag(tag, tag_timestamp_dict)
            for tag in advisory_record.relevant_tags
        ]
    )
    tags_to_calculate_reachability_from = tuple(
        dict.fromkeys(
            [tag for tag in tags_to_calculate_reachability_from if tag != None]
        )
    )  # none if there was no correct mapping with map_tag_onto_next_tag
    ...
    for tag in tags_to_calculate_reachability_from:
        since, until = database.timestamp_to_timestamp_interval(
            timestamp=tag_timestamp_dict[tag], days_before=days_before, days_after=0
        )
        reachable_commits[tag] = advisory_record.git_repo.get_commits(
            ancestors_of=tag, exclude_ancestors_of=None, since=since, until=until
        )

    # calculate scores
    candidate_commit_df["reachability_score"] = candidate_commit_df.apply(
        calculate_reachability_score,
        tag_timestamp_dict=tag_timestamp_dict,
        reachable_commits=reachable_commits,
        relevant_tags=tags_to_calculate_reachability_from,
...
def compute_referred_to_components(advisory_record, candidate_commit_df):
    ...
    """
    if (
        "commits_found_nvd" not in advisory_record.__dict__
        and "commits_found_adv" not in advisory_record.__dict__
    ):
        advisory_record.analyse_references()

    candidate_commit_df["referred_to_by_nvd"] = candidate_commit_df["id"].apply(
        if_commit_id_in_list, commits_found_list=advisory_record.commits_found_nvd
    )
    candidate_commit_df["referred_to_by_advisories"] = candidate_commit_df["id"].apply(
        if_commit_id_in_list, commits_found_list=advisory_record.commits_found_adv
    )
        ...
        if len(
            [
                ...
                for result in re.finditer(
                    "#\d+:?", " ".join(ast.literal_eval(message_column))
                )
            ]
        )
        > 0
    ...
    return (
        ...
        if len(
            [
                result.group(0)
                for result in re.finditer(
                    "\w+-\d+:?", " ".join(ast.literal_eval(message_column))
                )
            ]
        )
        > 0
...
def compute_ranking_vectors_for_advisory_records_with_db(
    advisory_record, vulnerability_specific_scaling
):
    """
    Core functionality:
    ...
    # candidate commit DF
    candidate_commit_df = pd.read_sql(
        "SELECT * FROM commits WHERE id IN {} AND repository_url = '{}'".format(
            tuple(
                advisory_record.candidate_commits
                + [advisory_record.candidate_commits[0]]
            ),
            advisory_record.repo_url,
        ),
        advisory_record.connection,
    )  # +[adisory_record.candidate_commits[0]] as SQL requires at least 2 items
    candidate_commit_df = check_preprocessing(advisory_record, candidate_commit_df)

    # creating the feature vectors
    candidate_commit_df["vulnerability_id_in_message"] = candidate_commit_df[
        "message"
    ].apply(if_vulnerability_id_in_message, vulnerability_id=advisory_record.id)
    candidate_commit_df["other_CVE_in_message"] = candidate_commit_df["message"].apply(
        if_other_vulnerability_id_in_message, vulnerability_id=advisory_record.id
    )

    # commit statistics
    candidate_commit_df["n_hunks"] = candidate_commit_df["hunks"].apply(
        hunks_to_n_hunks
    )
    candidate_commit_df["avg_hunk_size"] = candidate_commit_df["hunks"].apply(
        hunks_to_avg_hunk_size
    )
    candidate_commit_df["n_changed_files"] = candidate_commit_df["changed_files"].apply(
        changed_files_to_n_changed_files
    )

    # commit message references
    candidate_commit_df["git_issue_reference"] = candidate_commit_df["message"].apply(
        contains_issue_reference
    )
    candidate_commit_df["contains_jira_reference"] = candidate_commit_df[
        "message"
    ].apply(contains_jira_reference)

    # add path score
    path_tokens_list = extract_path_tokens_from_text(advisory_record.description)
    candidate_commit_df["path_similarity_score"] = candidate_commit_df[
        "changed_files"
    ].apply(compute_path_similarity_score, path_tokens_list=path_tokens_list)

    # drop columns that are no longer needed
    candidate_commit_df.drop(
        columns=[
            "repository_url",
            "hunks",
            "message",
    ...
    candidate_commit_df = compute_lexical_similarity_components(
        advisory_record, candidate_commit_df
    )
    candidate_commit_df = compute_commit_distance_components(
        advisory_record, candidate_commit_df, days_before=100
    )
    candidate_commit_df = compute_referred_to_components(
        advisory_record, candidate_commit_df
    )
    ...
    if vulnerability_specific_scaling:
        candidate_commit_df.iloc[:, 1:] = MinMaxScaler().fit_transform(
            candidate_commit_df.iloc[:, 1:]
        )

    # add the vulnerability timestamp as a somewhat normalized component to allow for learning
    candidate_commit_df["vulnerability_timestamp"] = [
        int(advisory_record.published_timestamp) / seconds_per_month
        for i in range(len(candidate_commit_df))
    ]
...
def get_first_commit_timestamp(repo_url, git_repo=None):
    ...
    first_commit_timestamp = commit._exec.run(
        'git show -s --format="%ct" ' + commit._id
    )[0][1:-1]
...
def rank_candidates(model, ranking_vectors):
    ...
    commit_scores = [
        (candidate, predictions[index][1]) for index, candidate in enumerate(commit_ids)
    ]
    return tuple(
        [
            commit[0]
            for commit in sorted(commit_scores, key=lambda x: x[1], reverse=True)
        ]
    )
...
def rank_ranking_vectors(ranking_vector_df, vulnerability_ids, model):
    ...
    for vulnerability in vulnerability_ids:
        if vulnerability in tuple(ranking_vector_df.vulnerability_id.unique()):
            subset = ranking_vector_df[
                ranking_vector_df.vulnerability_id == vulnerability
            ]
            ...
            commit_scores = [
                (candidate, predictions[index][1])
                for index, candidate in enumerate(commit_ids)
            ]
            ranked_vulnerability_candidates[vulnerability] = [
                commit[0]
                for commit in sorted(commit_scores, key=lambda x: x[1], reverse=True)
            ]
...
def evaluate_ranking(
    ranked_vulnerability_candidates,
    vulnerabilities,
    fix_commits_df,
    ...
    for vulnerability, ranked_candidates in ranked_vulnerability_candidates.items():
        if vulnerability in vulnerabilities:
            if vulnerability not in list(fix_commits_df.vulnerability_id):
                ...
                fix_commits = list(
                    fix_commits_df[
                        fix_commits_df.vulnerability_id == vulnerability
                    ].commit_id
                )
                fix_commit_positions = [
                    ranked_candidates.index(fix_commit)
                    for fix_commit in fix_commits
                    if fix_commit in ranked_candidates
                ]
            ...
            if len(fix_commit_positions) > 0:
                ...
                df.at[vulnerability, "mean_ranking_position"] = np.mean(
                    fix_commit_positions
                )
                df.at[vulnerability, "max_ranking_position"] = max(fix_commit_positions)
                df.at[vulnerability, "precision"] = len(fix_commit_positions) / (
                    1 + max(fix_commit_positions)
                )

                for position in k:
                    if len(fix_commit_positions) <= position:
                        df.at[vulnerability, "recall_at_{}".format(position)] = len(
                            [
                                ranked_position
                                for ranked_position in fix_commit_positions
                                if ranked_position < position
                            ]
                        ) / len(fix_commit_positions)
                    else:
                        df.at[vulnerability, "recall_at_{}".format(position)] = (
                            len(
                                [
                                    ranked_position
                                    for ranked_position in fix_commit_positions
    ...
    if verbose:
        print(
            "{} / {} do not have a known fix among the candidates (and {} do not have a fix in the data).".format(
                no_fixes_found_count, len(vulnerabilities), len(no_fixes_count)
            )
        )
    return df
...
def create_train_and_test_datasets(
    ...
    for vulnerability in vulnerability_ids:
        vulnerability_ranking_vectors = (
            ranking_vector_df[ranking_vector_df.vulnerability_id == vulnerability]
            .set_index("commit_id")
            .drop(columns=["vulnerability_id"])
        )
        fix_commits = [
            commit_id
            ...
            if commit_id in list(vulnerability_ranking_vectors.index)
        ]

        non_fix_commits = [
            commit_id
            for commit_id in list(vulnerability_ranking_vectors.index)
            if commit_id not in fix_commits
        ]

        # add non fix commits
        n_fix_commits = len(fix_commits)
        random.shuffle(non_fix_commits)
        commits_to_select = non_fix_commits[
            : negative_samples_multiplier * n_fix_commits
        ]
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        subset, labels, test_size=test_size, random_state=random_state
    )
...
def analyse_ranking_results(ranking_results_df, method, train_or_test, k=[5, 10, 20]):
    ...
    for position in k:
        fixes_in_top_k = len(
            ranking_results_df[
                (ranking_results_df.max_ranking_position < position)
                & (ranking_results_df.n_fix_commits <= position)
            ]
        ) + len(
            ranking_results_df[
                (
                    ranking_results_df.max_ranking_position
                    < ranking_results_df.n_fix_commits
                )
                & (ranking_results_df.n_fix_commits > position)
            ]
        )
        df.at[0, "recall_at_{}".format(position)] = round(
            fixes_in_top_k / len(ranking_results_df) * 100, 2
        )
    return df


def analyze_cross_validate_df(
    cross_validate_df, validation_methods_to_keep=["all", "best"]
):
    """
    ...
    # # process the entries
    cross_validate_df["validation_method"] = cross_validate_df["method"].apply(
        lambda x: x.split("-")[1]
    )
    cross_validate_df["method"] = cross_validate_df["method"].apply(
        lambda x: x.split("-")[0]
    )
    cross_validate_df["avg_precision"] = cross_validate_df["avg_precision"].apply(
        lambda x: round(x * 100, 2)
    )

    cross_validate_df = cross_validate_df[
        cross_validate_df.validation_method.isin(validation_methods_to_keep)
    ]

    if len(validation_methods_to_keep) > 1:
        # #select the relevant rows
        cross_validate_df.drop(columns=["index"], inplace=True)
        grouped_cross_validate_df = cross_validate_df.groupby(
            ["train_or_test", "method", "validation_method"]
        ).mean()

        for row in grouped_cross_validate_df.iterrows():
            subset = cross_validate_df[
                (cross_validate_df.method == row[0][1])
                & (cross_validate_df.validation_method == row[0][2])
                & (cross_validate_df.train_or_test == row[0][0])
            ]
            for column in columns:
                column_values_dict[column].append(
                    "{} ({})".format(
                        round(row[1][column], 2), round(subset[column].std(), 2)
                    )
                )
