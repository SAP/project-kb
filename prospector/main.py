...
# COLUMNS
vulnerability_specific_columns = ['message_score', 'changed_files_score',
                                  'git_diff_score', 'message_score_reference_content', 'changed_files_score_code_tokens']
universal_columns = ['n_hunks', 'avg_hunk_size',
                     'n_changed_files', 'vulnerability_timestamp']
columns_to_drop = ['path_similarity_score', 'git_diff_score_code_tokens', 'message_score_code_tokens',
                   'changed_files_score_reference_content', 'git_diff_score_reference_content']


@plac.annotations(
    vulnerability_id=plac.Annotation(
        "A vulnerability ID (typically a CVE)", type=str),
    verbose=plac.Annotation("Verbose mode", 'flag', 'v'),
    description=plac.Annotation(
        "The vulnerability description", "option", type=str),
    published_timestamp=plac.Annotation(
        "The timestamp at which the vulnerability is published (int)", "option", type=int),
    ...
    references=plac.Annotation(
        "A list of references that provide additional information on the vulnerability", "option", type=list),
    k=plac.Annotation("The number of candidates to show", "option", type=int),
    vulnerability_specific_scaling=plac.Annotation(
        "To apply vulnerability specific scaling, or apply the pre-trained MinMaxScaler", "option", type=bool)
)
def main(vulnerability_id, verbose=False,
         description=None,
         published_timestamp=None, repo_url=None, project_name=None, references=None, k=10, vulnerability_specific_scaling=False):
    ...
    if database.if_new_vulnerability(vulnerabilities_cursor, vulnerability_id) == False:
        vulnerability = vulnerabilities_cursor.execute(
            "SELECT * FROM vulnerabilities WHERE vulnerability_id = :vulnerability_id", {'vulnerability_id': vulnerability_id}).fetchone()
        ...
        if references != None:
            database.add_vulnerability_references_to_database(
                vulnerabilities_connection, vulnerability_id, references, driver=None, verbose=verbose)
        else:
            references = references if references != None else [nvd_reference['url'] for nvd_reference in vulnerabilities_cursor.execute(
                "SELECT url FROM vulnerability_references WHERE vulnerability_id = :vulnerability_id", {'vulnerability_id': vulnerability_id})]
        ...
        if description == None or published_timestamp == None or references == None:
            ...
            if description == None:
                if nvd_description == None:
                    ...
                    print("Since the provided vulnerability ID {} cannot be found in the NVD, you must provide a vulnerability description manually.".format(
                        vulnerability_id))
                    description = input()
            ...
            if published_timestamp == None:
                if nvd_published_timestamp == None:
                    ...
                    print("Since the provided vulnerability ID {} cannot be found in the NVD, you must provide a vulnerability timestamp manually.".format(
                        vulnerability_id))
            ...
            if references == None:
                if nvd_references == None:
                    # raise ValueError("Since the provided vulnerability ID {} cannot be found in the NVD, you must provide a vulnerability description manually.".format(vulnerability_id))
                    print("Since the provided vulnerability ID {} cannot be found in the NVD, you must provide a vulnerability references manually (comma seperated).".format(
                        vulnerability_id))
                    references = input()
    ...
    references_content = rank.extract_n_most_occurring_words(rank.remove_forbidden_words_from_string(
        string=' '.join(references_content), forbidden_words=rank.reference_stopwords + project_name.split(' ')), n=20)

    # @TODO: now adding all advisory references --> change to only using the provided references
    advisory_references = [advisory_reference['url'] for advisory_reference in vulnerabilities_cursor.execute(
        "SELECT url FROM advisory_references WHERE vulnerability_id = :vulnerability_id", {'vulnerability_id': vulnerability_id})]
    ...
    if verbose:
        ...
        print(
            " - References content extracted: {}".format(advisory_record.references_content))
    ...
    if vulnerability_specific_scaling == False:
        ...
        advisory_record.ranking_vectors[vulnerability_specific_columns] = MinMaxScaler(
        ).fit_transform(advisory_record.ranking_vectors[vulnerability_specific_columns])
        advisory_record.ranking_vectors[universal_columns] = universal_columns_scaler.transform(
    ...
    output = advisory_record_to_output(
...
def advisory_record_to_output(advisory_record, model, prospector_cursor, k=20):
    ...
    string += '\n\nWEIGHTS (Logistic Regression Coefficients):\n{}'.format(pd.DataFrame({'feature': advisory_record.ranking_vectors.columns,
                                                                                         'importance': model.coef_[0]}).set_index('feature').sort_values('importance', ascending=False).transpose().loc['importance'])
    ...
    for i in range(k):
        ...
        string += '\n - Tag(s): {}'.format(Commit(advisory_record.git_repo,
        ...
        commit = prospector_cursor.execute("SELECT message, changed_files, preprocessed_message FROM commits WHERE id = :commit_id AND repository_url = :repo_url", {
                                           'commit_id': advisory_record.ranked_candidate_commits[i], 'repo_url': advisory_record.repo_url}).fetchone()
        # commit['preprocessed_message']#
        commit_message = str(
            ' '.join(ast.literal_eval(str(commit['message']))))
