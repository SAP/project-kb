    ...
    try:
        if state.vulnerabilities_df == None:
            (
                ...
                state.advisory_references_df,
                state.tags_df,
                state.repository_url_df,
                state.fixes_df,
            ) = load_vulnerabilities()
    except:
        pass

    pages = {"Dashboard": dashboard_page, "Inspect": inspect_page}
...
def load_vulnerabilities():
    ...
    ) = database.connect_with_vulnerabilities_database(vulnerabilities_db_path)

    print("Reading vulnerabilities")
    vulnerabilities_df = pd.read_sql(
        "SELECT * FROM vulnerabilities", vulnerabilities_connection
    ...
    for i, repo_url in enumerate(list(vulnerabilities_df.repo_url.unique())):
        ...
        repository_url_df.at[i, "project_name"] = rank.simpler_filter_text(
            re.sub("^https?://|[^\w]", " ", repo_url)
        ).lower()
    repository_url_df["project_name"] = repository_url_df.apply(
        lambda x: " ".join(
            [
                token
                for token in x["project_name"].split(" ")
...
def map_description_to_repository_url(
    vulnerability_id, description, vulnerabilities_df, repository_url_df
):
...
def get_vulnerability_data(vulnerability_id, vulnerabilities_df, db_references_df):
    if type(vulnerabilities_df) != type(None) and vulnerability_id in list(
        ...
        (
            repo_url,
            cve_description,
            cve_published_timestamp,
            preprocessed_description,
        ) = vulnerabilities_df.loc[vulnerability_id]
        # cve_project_name = ' '.join(re.split('/|-|\.', cve_repo_url.lstrip('https?://')))
        references = list(
            db_references_df[db_references_df.vulnerability_id == vulnerability_id].url
        )
    else:
        (
            cve_description,
...
def dashboard_page(state):
    ...
    state.vulnerability_id = st.text_input(
        ...
        value=state.vulnerability_id if state.vulnerability_id else "",
    ).upper()
    ...
    if state.vulnerability_id:
        try:
            (
                ...
                state.vulnerability_id, state.vulnerabilities_df, state.db_references_df
            )
        except:
            ...
            cve_description, cve_published_timestamp, preprocessed_description = (
                "",
                time.time(),
                None,
            )
    ...
    vulnerability_description = st.text_area(
        "Vulnerability description", value=cve_description
    )
    project_name = st.text_input(
        ...
        value=" ".join(
            [
                token.text
                for token in nlp(vulnerability_description)
                if token.tag_ == "NNP"
            ]
        ),
    )
    ...
    published_date = st.date_input(
        ...
        value=datetime.fromtimestamp(int(cve_published_timestamp)),
    )
    published_timestamp = int(time.mktime(published_date.timetuple()))

    state.advisory_record_confirmed = (
        st.button("CONFIRM ADVISORY RECORD")
        if not state.advisory_record_confirmed
        else True
    )
    if state.advisory_record_confirmed:
        ...
        if type(state.vulnerabilities_df) == type(
            None
        ) or state.vulnerability_id not in list(state.vulnerabilities_df.index):
            (
                vulnerabilities_connection,
                vulnerabilities_cursor,
            ) = database.connect_with_vulnerabilities_database(vulnerabilities_db_path)
            database.add_vulnerability_to_database(
                vulnerabilities_connection,
            ...
            if len(references) == 0:
                try:
                    (
                        ...
                        cve_published_timestamp,
                        references,
                    ) = database.extract_nvd_content(state.vulnerability_id)
        ...
        references = [
            state.db_references_df.at[index, "url"]
            for index in state.db_references_df[
                state.db_references_df.vulnerability_id == state.vulnerability_id
            ].index
        ]

        advisory_references = list(
            state.advisory_references_df[
                state.advisory_references_df.vulnerability_id == state.vulnerability_id
            ].url
        )

        # allow the user to influence the filtering
        state.advanced_settings = st.checkbox(
        ...
        if state.advanced_settings:
            ...
            first_commit_date, today = (
                datetime.fromtimestamp(int(first_commit_timestamp)).date(),
                datetime.fromtimestamp(int(time.time())).date(),
            )
            lower_bound = (
                published_date - timedelta(days=730)
                if published_date - timedelta(days=730) > first_commit_date
                else first_commit_date
            )
            upper_bound = (
                published_date + timedelta(days=100)
                if published_date + timedelta(days=100) < today
                else today
            )

            since, until = st.slider(
                "Published date based interval",
            ...
            selected_references = st.multiselect(
                "Advisory references", tuple(references), default=tuple(references)
            )
        ...
        if st.button("Search prospects!"):
            ...
            st.write(
                "Showing the top {} candidates from {} candidates considered".format(
                    k, len(advisory_record.ranking_vectors)
                )
            )
            st.write(output)
...
def inspect_page(state):
    ...
    if st.button("Select random vulnerability"):
        random_vulnerability = state.vulnerabilities_df.index[
            random.randint(0, len(state.vulnerabilities_df) - 1)
        ]
        st.subheader(random_vulnerability)
        st.write(
            "Affected repository:",
            state.vulnerabilities_df.at[random_vulnerability, "repo_url"],
        )
        st.write(
            "Vulnerability description:",
            state.vulnerabilities_df.at[random_vulnerability, "description"],
        )
        st.write(
            "Published date:",
            datetime.fromtimestamp(
                int(state.vulnerabilities_df.at[random_vulnerability, "published_date"])
            ),
        )
