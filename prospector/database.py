    ...
    since = str(
        int(
            (
                ...
                - datetime.timedelta(days=int(days_before))
            ).timestamp()
        )
    )
    until = str(
        int(
            (
                datetime.datetime.fromtimestamp(int(timestamp))
                + datetime.timedelta(days=int(days_after))
            ).timestamp()
        )
    )
    return since, until
...
def get_commit_ids_between_timestamp_interval(
    since, until, git_repo=None, repository_url=None
):
    """
...
def get_hunks_from_diff(diff_lines):
    ...
    if type(diff_lines) == str:
        try:
            ...
            print(
                "The diff_lines is in string format, while a list is expected. ast.literal_eval() is used to transform it."
            )
        except:
...
def get_changed_files_from_diff(diff_lines):
    ...
    return list(
        dict.fromkeys(
            [
                changed_file.lstrip("a/, b/")
                for line in diff_lines
                if line.startswith("diff --git")
                for changed_file in line.lstrip("diff --git ").split(" ")
            ]
        )
    )
...
def connect_with_database(path, as_row_factory=True, verbose=True):
    ...
    try:
        ...
        if verbose:
            print("Opening database on path {}".format(path))
    except:
        if verbose:
            print("Creating new database on path {}".format(path))
...
def create_commits_table(connection):
    ...
    with connection:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS repository_index ON commits(repository_url)"
        )
        cursor.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS commit_repository_index ON commits(id, repository_url)"
        )
...
def add_commits_to_database(
    ...
    commits_already_in_the_db = list(
        pd.read_sql(
            "SELECT id FROM commits WHERE id IN {} and repository_url = '{}'".format(
                tuple(commit_ids + [commit_ids[0]]), repository_url
            ),
            connection,
        ).id
    )
    ...
    if verbose:
        print(
            "    {} / {} are already in the database, now adding the rest.".format(
                len(commits_already_in_the_db), len(commit_ids)
            )
        )
        ...
        try:
            ...
            preprocessed_diff = rank.simpler_filter_text(
                re.sub(
                    "[^A-Za-z0-9]+",
                    " ",
                    " ".join(rank.extract_relevant_lines_from_commit_diff(diff)),
                )
            )
            ...
            with connection:
                cursor.execute(
                    "INSERT INTO commits VALUES (:repository_url, :id, :timestamp, :message, :changed_files, :diff, :hunks, :commit_message_reference_content, :preprocessed_message, :preprocessed_diff, :preprocessed_changed_files, :preprocessed_commit_message_reference_content)",
                    {
                        ...
                        "diff": str(diff),
                        "hunks": str(hunks),
                        "commit_message_reference_content": commit_message_reference_content,
                        "preprocessed_message": preprocessed_message,
                        "preprocessed_diff": preprocessed_diff,
                        "preprocessed_changed_files": preprocessed_changed_files,
                        "preprocessed_commit_message_reference_content": preprocessed_commit_message_reference_content,
                    },
                )
        except:
            print("    Failed to add commit {}".format(commit_id))
    if verbose:
        print("    All commits have been added to the database.")
    cursor.close()
    return
...
def create_repositories_table(connection):
    ...
    with connection:
        ...
        cursor.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS repository_index ON repositories(repo_url)"
        )
...
def add_repository_to_database(connection, repo_url, project_name, verbose=True):
    ...
    if (
        cursor.execute(
            "SELECT EXISTS(SELECT 1 FROM repositories WHERE repo_url = (?) LIMIT 1) AS 'exists';",
            (repo_url,),
        ).fetchone()[0]
        == 0
    ):
        with connection:
            cursor.execute(
                "INSERT INTO repositories VALUES (:repo_url, :project_name)",
                {"repo_url": repo_url, "project_name": project_name},
            )
        if verbose:
            print("    Successfully added repository to the repositories table.")
    elif verbose:
...
def create_tags_table(connection):
    ...
    with connection:
        ...
        cursor.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS tags_index ON tags(tag, repo_url)"
        )
...
def add_tags_to_database(
    connection, tags=None, git_repo=None, repo_url=None, verbose=True
):
    """
    ...
    cursor.execute(
        "SELECT tag FROM tags WHERE repo_url = :repo_url AND tag IN {}".format(
            tuple(tags)
        ),
        {"repo_url": repo_url},
    )
    tags_already_in_the_db = list(
        pd.read_sql(
            "SELECT tag FROM tags WHERE tag IN {} and repo_url = '{}'".format(
                tuple(tags + [tags[0]]), repo_url
            ),
            connection,
        ).tag
    )
    ...
    for tag in tqdm(tags_to_add):
        try:
            ...
            # add to database
            cursor.execute(
                "INSERT INTO tags VALUES (:tag, :repo_url, :tag_timestamp)",
    ...
    if verbose:
        print(
            "    {} / {} tags were already in the database and added the rest.".format(
                len(tags_already_in_the_db), len(tags)
            )
        )
    cursor.close()
    return
...
def connect_with_vulnerabilities_database(path, as_row_factory=True, verbose=True):
    ...
    try:
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS vulnerability_id ON vulnerabilities(vulnerability_id)"
        )
    except:
    ...
    if as_row_factory and verbose:
        print(
            "Database contains {} vulnerabilities".format(
                cursor.fetchone()["COUNT(vulnerability_id)"]
            )
        )
    elif verbose:
...
def if_new_vulnerability(cursor, vulnerability_id):
    ...
    if (
        cursor.execute(
            "SELECT EXISTS(SELECT 1 FROM vulnerabilities WHERE vulnerability_id = :vulnerability_id LIMIT 1) AS 'exists';",
...
def extract_nvd_content(vulnerability_id):
    ...
    """
    vulnerability_id = re.sub(
        "https://|nvd.nist.gov/vuln/detail/", "", vulnerability_id
    )  # someone might provide an NVD url
    nist_nvd_request_url = "https://services.nvd.nist.gov/rest/json/cve/1.0/"
    r = requests.get(nist_nvd_request_url + vulnerability_id)
    assert r.ok == True, "vulnerability ID {} is not in the NVD".format(
        vulnerability_id
    )
    cve_content = r.json()

    description = cve_content["result"]["CVE_Items"][0]["cve"]["description"][
        "description_data"
    ][0]["value"]
    published_timestamp = int(
        time.mktime(
            datetime.datetime.strptime(
                cve_content["result"]["CVE_Items"][0]["publishedDate"].split("T")[0],
                "%Y-%m-%d",
            ).timetuple()
        )
    )
    references = [
        reference["url"]
        for reference in cve_content["result"]["CVE_Items"][0]["cve"]["references"][
            "reference_data"
        ]
    ]
...
def add_vulnerability_to_database(
    connection,
    vulnerability_id,
    repo_url,
    description=None,
    ...
    if if_new_vulnerability(cursor, vulnerability_id):
        ...
        if description == None or published_timestamp == None or references == None:
            try:
                (
                    ...
                    nvd_references,
                ) = extract_nvd_content(vulnerability_id)
            ...
            if published_timestamp == None:
                if nvd_published_timestamp == None:
                    raise ValueError(
                        "Since the provided vulnerability ID {} cannot be found in the NVD, you must provide a vulnerability published timestamp manually.".format(
                            vulnerability_id
                        )
                    )
                else:
            ...
            if references == None:
                if nvd_references == None:
                    raise ValueError(
                        "Since the provided vulnerability ID {} cannot be found in the NVD, you must provide a advisory references manually.".format(
                            vulnerability_id
                        )
                    )
                else:
        ...
        if references != None and len(references) > 0:
            add_vulnerability_references_to_database(
                connection, vulnerability_id, references, driver=driver, verbose=verbose
            )
    elif verbose:
        print(
            "    There is already a vulnerability with ID {} in the database".format(
                vulnerability_id
            )
        )
    cursor.close()
...
def create_vulnerability_references_table(connection):
    ...
    with connection:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS reference_index ON vulnerability_references(url)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS vulnerability_id_index ON vulnerability_references(vulnerability_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS reference_and_vulnerability_id_index ON vulnerability_references(url, vulnerability_id)"
        )
...
def add_vulnerability_references_to_database(
    connection, vulnerability_id, references, driver=None, verbose=True
):
    """
    Input:
        connection (sqlite3.connection): the connection with the database
...
def create_advisory_references_table(connection, verbose=True):
    ...
    with connection:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS reference_index ON advisory_references(url)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS vulnerability_id_index ON advisory_references(vulnerability_id)"
        )
        cursor.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS reference_and_vulnerability_id_index ON advisory_references(url, vulnerability_id)"
        )
    if verbose:
        print("    Table advisory_references successfully created")
    cursor.close()
    return


def add_advisory_references_to_database(
    connection, vulnerability_id, references, verbose=True
):
    """
    # @TODO: add a column that contains the URL from which this URL is obtained
    ...
    for reference in references:
        ...
        if (
            cursor.execute(
                "SELECT EXISTS(SELECT 1 FROM advisory_references WHERE  url = :url AND vulnerability_id = :vulnerability_id LIMIT 1) AS 'exists';",
                {"url": reference, "vulnerability_id": vulnerability_id},
            ).fetchone()["exists"]
            == 0
        ):
            ...
            with connection:
                cursor.execute(
                    "INSERT INTO advisory_references VALUES (:url, :vulnerability_id)",
                    {"url": reference, "vulnerability_id": vulnerability_id},
                )
        elif verbose:
            print("    reference {} is already in the db".format(reference))
...
def create_vulnerability_fixes_table(connection):
    ...
    with connection:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS commit_index ON fix_commits(commit_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS vulnerability_id_index ON fix_commits(vulnerability_id)"
        )
...
def add_vulnerability_fixes_to_database(
    connection, vulnerability_id, commit_ids, repo_url, verbose=True
):
    """
    Input:
        connection (sqlite3.connection): the connection with the database
    ...
    for commit_id in commit_ids:
        ...
        if (
            cursor.execute(
                "SELECT EXISTS(SELECT 1 FROM fix_commits WHERE  commit_id = :commit_id AND vulnerability_id = :vulnerability_id LIMIT 1) AS 'exists';",
                {"commit_id": commit_id, "vulnerability_id": vulnerability_id},
            ).fetchone()["exists"]
            == 0
        ):
            ...
            with connection:
                cursor.execute(
                    "INSERT INTO fix_commits VALUES (:commit_id, :vulnerability_id, :repo_url)",
                    {
                        "commit_id": commit_id,
                        "vulnerability_id": vulnerability_id,
                        "repo_url": repo_url,
            ...
            print(
                "    Fix {} for vulnerability {} is already in the db".format(
                    commit_id, vulnerability_id
                )
            )
    cursor.close()
    return
