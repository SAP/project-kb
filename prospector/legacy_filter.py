...
def filter_description(description):
    ...
    for sentence in list(doc.sents):
        ...
        for token_index, token in enumerate(sentence):
            if (
                token.tag_ in ["IN", "RBR", "RB", "VBG", "JJR"]
                or token.is_alpha == False
                and token.is_punct == False
                or token.lemma_ in ["inclusive"]
            ):
                relevant_tokens_in_sentence.append(token)

            # only keep punctuation before  the version
            if token.text in ["-"] and "d.d" in sentence[token_index - 1].shape_:
                relevant_tokens_in_sentence.append(token)

        for token in relevant_tokens_in_sentence:
            if (
                "d.d" in token.shape_
                and token.text[-1].lower() != "x"
                and relevant_tokens_in_sentence not in relevant_sentences
            ):

                # apply a padding
                relevant_tokens_in_sentence = (
                    [nlp("padding")[0]]
                    + relevant_tokens_in_sentence
                    + [nlp("padding")[0]]
                )
...
def retreive_all_versions_from_description(description):
    ...
    if type(description) != str:
        raise TypeError(
            "The provided description should be a str data type but is of type {}.".format(
                type(description)
            )
        )
...
def get_commits_between_interval_tags(intervals_tags, git_repo=None, repo_url=None):
    ...
    for interval_tags in intervals_tags:
        ...
        candidates = git_repo.get_commits(
            since=time_1,
            ancestors_of=cid_2,
            exclude_ancestors_of=cid_1,
            filter_files="*",
        )
...
def map_advisory_record_onto_candidate_commits(advisory_record):
    ...
    if "repo_url" not in advisory_record or "description" not in advisory_record:
        raise ValueError(
            "advisory record should contain variables repo_url and description."
        )
    ...
    versions_in_description = retreive_all_versions_from_description(
    ...
    for version in versions_in_description:
        ...
        for version_interval_tags in version_to_wide_interval_tags(
            tags, version, git_repo
        ):
            if (
                version_interval_tags not in tag_intervals
                and version_interval_tags != None
            ):
                tag_intervals.append(version_interval_tags)

    # map the list of intervals onto candidate commits
    candidate_commits = get_commits_between_interval_tags(
        tag_intervals, git_repo=git_repo
    )
...
# makes use of the DB
def filter_commits_on_files_changed_extensions(
    commit_ids, connection, return_irrelevant_commits=False, verbose=True
):
    """
    ...
    """
    commit_changed_files_df = pd.read_sql_query(
        "SELECT id, changed_files FROM commits WHERE id in {}".format(
            tuple(commit_ids + [commit_ids[0]])
        ),
        connection,
    )
    commit_changed_files_df["extensions"] = commit_changed_files_df[
        "changed_files"
    ].apply(extract_extensions_from_changed_files)
    commit_changed_files_df["changes_relevant_file"] = commit_changed_files_df[
        "extensions"
    ].apply(changes_relevant_file)
    irrelevant_commits_based_on_extension = list(
        commit_changed_files_df[
            commit_changed_files_df.changes_relevant_file == False
        ].id
    )

    if verbose:
        print(
            "    {} / {} candidates do not change a relevant file based on the extension".format(
                len(irrelevant_commits_based_on_extension), len(commit_ids)
            )
        )
    ...
    return [
        ...
        if commit_id not in irrelevant_commits_based_on_extension
    ]
