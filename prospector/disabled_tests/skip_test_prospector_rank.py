...
def example_commit_content():
    example_commit_content = {
        ...
        "diff": [
            ...
            "index 507498f..66834c6 100644",
            "--- a/CHANGELOG.md",
            "+++ b/CHANGELOG.md",
            "@@ -11,2 +11,3 @@ A pre-release can be downloaded from https://ci.jenkins.io/job/Plugins/job/docke",
...
@pytest.mark.parametrize("token, error", [(["CamelCaseToken."], TypeError)])
...
@pytest.mark.exception_handling
@pytest.mark.parametrize("token, error", [(["snake_case_token"], TypeError)])
...
def test_text_into_chunks_with_commit(example_commit_content):
    ...
    chunks = rank.text_into_chunks(
        text=example_commit_content["message"], chunk_size=100
    )
    assert len(chunks) == 1
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["Check what happens", 12345, "When there are integers in the list"],
            TypeError,
        ),
    ],
)
...
def test_filter_text_2():
    ...
    assert rank.filter_text(text, as_list=True) == [
        "example",
        "sentence",
        "test",
        "functionality",
...
@pytest.mark.text_preprocessing
@pytest.mark.parametrize(
    "text, remove_duplicates, case_sensitive, lemmatize, result",
    [
        ("This is an example.", True, False, True, "example"),
        ("This is an example: Example.", False, True, False, "example Example"),
        (
            "This is an example. In this example, example occurs more than once.",
            True,
            False,
            True,
            "example occur",
...
def test_filter_text(text, remove_duplicates, case_sensitive, lemmatize, result):
    assert result == rank.filter_text(
        text,
        as_tokens=False,
        as_list=False,
...
def test_simpler_filter_text(example_commit_content):
    ...
    """
    assert (
        rank.simpler_filter_text(text=example_commit_content["message"])
        == "add changelog merge"
    )
    assert (
        rank.simpler_filter_text(text=" ".join(example_commit_content["message"]))
        == "add changelog merge"
    )
    assert (
...
def test_extract_relevant_lines_from_commit_diff(example_commit_content):
    assert (
        len(
            rank.extract_relevant_lines_from_commit_diff(
                git_diff=example_commit_content["diff"], max_lines=10000
            )
        )
        == 2
    )
    assert (
        len(
            rank.extract_relevant_lines_from_commit_diff(
                git_diff=example_commit_content["diff"], max_lines=1
            )
        )
        == 1
    )
    assert (
        rank.extract_relevant_lines_from_commit_diff(
            git_diff=example_commit_content["diff"]
        )[1]
        == "+* Enhancement: templates can now specify cpu period and cpu quota [#795](https://github.com/jenkinsci/docker-plugin/issues/795)"
    )
...
def test_extract_relevant_lines_from_commit_errors(example_commit_content):
    ...
    with pytest.raises(TypeError):
        rank.extract_relevant_lines_from_commit_diff(
            git_diff=" ".join(example_commit_content["diff"])
        )
...
def test_extract_n_most_occurring_words(example_commit_content):
    assert (
        rank.extract_n_most_occurring_words(
            rank.simpler_filter_text(
    ...
    assert (
        rank.extract_n_most_occurring_words(
            rank.simpler_filter_text(example_commit_content["message"]), n=1
        )
        == "add"
    )
    assert (
        rank.extract_n_most_occurring_words(
            rank.simpler_filter_text(" ".join(example_commit_content["message"])), n=1
        )
        == "add"
    )
...
def test_find_references(example_commit_content):
    ...
    assert (
        "https://github.com/jenkinsci/docker-plugin/issues/802"
        in rank.find_references(example_commit_content["diff"])
    )
