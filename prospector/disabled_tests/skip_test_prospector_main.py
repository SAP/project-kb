...
) = database.connect_with_vulnerabilities_database("test-vulnerabilities.db")
...
def test_database_coverage(example_vulnerability):
    ...
    assert (
        ...
        ).fetchone()["COUNT(vulnerability_id)"]
        == 1
    )
    ...
    database.add_vulnerability_references_to_database(
        vulnerabilities_connection,
        example_vulnerability["vulnerability_id"],
        example_vulnerability["nvd_references"],
    )
    assert (
        vulnerabilities_cursor.execute(
            "SELECT COUNT(url) FROM vulnerability_references WHERE vulnerability_id = :vulnerability_id;",
            {"vulnerability_id": example_vulnerability["vulnerability_id"]},
        ).fetchone()["COUNT(url)"]
        == 2
    )
    assert (
        vulnerabilities_cursor.execute(
            "SELECT COUNT(url) FROM advisory_references WHERE vulnerability_id = :vulnerability_id;",
            {"vulnerability_id": example_vulnerability["vulnerability_id"]},
        ).fetchone()["COUNT(url)"]
        == 35
    )

    database.add_vulnerability_fixes_to_database(
        vulnerabilities_connection,
        example_vulnerability["vulnerability_id"],
        example_vulnerability["fix_commits"],
        example_vulnerability["repo_url"],
    )
    assert (
        vulnerabilities_cursor.execute(
            "SELECT COUNT(commit_id) FROM fix_commits WHERE vulnerability_id = :vulnerability_id;",
            {"vulnerability_id": example_vulnerability["vulnerability_id"]},
        ).fetchone()["COUNT(commit_id)"]
        == 1
    )
...
def test_extracting_reference_content(example_vulnerability):
    vulnerabilities_cursor.execute(
        "SELECT url, preprocessed_content FROM vulnerability_references WHERE vulnerability_id = :vulnerability_id",
        {"vulnerability_id": example_vulnerability["vulnerability_id"]},
    )
    nvd_references = {
        nvd_reference["url"]: nvd_reference["preprocessed_content"]
        for nvd_reference in vulnerabilities_cursor
    }
    nvd_references_content = rank.extract_n_most_occurring_words(
        rank.remove_project_name_from_string(
            example_vulnerability["project_name"], " ".join(nvd_references.values())
        ),
        n=20,
    )
...
def test_commit_coverage(example_vulnerability):
    ...
    advisory_record = rank.Advisory_record(
        ...
        preprocessed_vulnerability_description=example_vulnerability[
            "preprocessed_description"
        ]
        + example_vulnerability["nvd_reference_content"],
    )
