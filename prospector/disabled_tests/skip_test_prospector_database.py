...
def example_vulnerability_git_repo(example_vulnerability):
    example_vulnerability_git_repo = Git(
        example_vulnerability["repo_url"], cache_path=GIT_CACHE
    )
...
@pytest.mark.database
def test_get_commit_ids_between_timestamp_interval(
    example_vulnerability, example_vulnerability_git_repo
):
    # with a git_repo provided
    commit_ids_to_add = database.get_commit_ids_between_timestamp_interval(
        since=1457823600,
        until=1529532000,
    ...
    assert commit_ids_to_add[0], commit_ids_to_add[-1] == (
        "e4c9304553f2868f67556644f5831eba60cf2c34",
        "be96e51aa2ba68e0f230ff954a3d47f1c15c7a95",
    )
...
def test_get_commit_ids_between_timestamp_interval_errors(
    example_vulnerability, example_vulnerability_git_repo
):
    with pytest.raises(ValueError):
        database.get_commit_ids_between_timestamp_interval(
            since="1457823600", until="1529532000", git_repo=None, repository_url=None
        )
    with pytest.raises(ValueError):
        database.get_commit_ids_between_timestamp_interval(
            since="1529532000", until="1457823600", git_repo=None, repository_url=None
        )
...
def test_extract_commit_message_reference_content(example_vulnerability):
    ...
    message = [
        "Merge pull request #118 from dwnusbaum/add-comments",
        "Add comments to clarify the HtmlUnit-based tests in ManualConditionTest",
    ]
    reference_content = database.extract_commit_message_reference_content(
        message, repo_url=example_vulnerability["repo_url"]
    )
    ...
    assert (
        len(
            database.extract_commit_message_reference_content(
                "message", repo_url=example_vulnerability["repo_url"]
            )
        )
        == 0
...
def test_add_commits_to_database(example_vulnerability, example_vulnerability_git_repo):
    ...
    assert (
        row["repository_url"] == "https://github.com/jenkinsci/promoted-builds-plugin"
    )
