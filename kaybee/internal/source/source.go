...
// A Source is a source of Statements
type Source struct {
	URL    string          `yaml:"url"`
...
// NewSource creates a new source and updates the appropriate path
func NewSource(URL string, branch string, path string, strict bool, rank int, targetDir string) Source {
	r := Source{
		URL:    URL,
		Branch: branch,
		Path:   path,
...
func (r *Source) Fetch(verbose bool) {
	var err error
	if verbose {
...
func (r *Source) Statements() ([]model.Statement, error) {
	s := []model.Statement{}
	...
	for _, item := range files {
		if item.IsDir() {
			subitems, _ := ioutil.ReadDir(filepath.Join(r.Path, filesystem.DataPath, item.Name()))
...
func (r *Source) VerifyCommit(o *object.Commit) bool {
...
func (r *Source) getRepoPath() (string, error) {
...
func (r *Source) getLicensePath() (string, error) {
...
func (r *Source) resetPullTimestamp() {
